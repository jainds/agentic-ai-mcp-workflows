...
def test_data_agent_operations():
    ...
    # Mock the enterprise API calls to avoid network errors
    with patch.object(data_agent.user_api, 'get') as mock_user_api, \
         patch.object(data_agent.policy_api, 'get') as mock_policy_api, \
         patch.object(data_agent.claims_api, 'post') as mock_claims_post, \
         patch.object(data_agent.claims_api, 'get') as mock_claims_get:
        
        # Set up mock responses
        mock_user_api.return_value = None  # Will trigger mock data generation
        mock_policy_api.return_value = None  # Will trigger mock data generation
        mock_claims_post.return_value = None  # Will trigger mock data generation
        mock_claims_get.return_value = None  # Will trigger mock data generation
