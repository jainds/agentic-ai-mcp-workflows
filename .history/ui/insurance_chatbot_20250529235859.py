...
class InsuranceAIClient:
    ...
    def _demo_response_with_a2a_simulation(self, user_message: str, customer_id: str, customer_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "claim" in user_message.lower():
            orchestration_steps = [
                f"📋 Claims Agent (Domain) analyzing claim request for {customer_data['name']}",
            ...
            # Simulate additional API calls with customer context
            self._simulate_api_call("data-agent", f"/mcp/claims/create", "POST", 200, 450, customer_id)
            self._simulate_api_call("data-agent", f"/mcp/customer/get", "GET", 200, 230, customer_id)
            self._simulate_api_call("notification-agent", f"/mcp/email/send", "POST", 200, 340, customer_id)
            
            # Use actual customer data in response
            customer_policies = customer_data.get('policies', [])
            main_policy = customer_policies[0] if customer_policies else "POL-AUTO-123456"
            
            demo_response = f"""I've processed your claim request through our A2A orchestration system for **{customer_data['name']}** (ID: {customer_id}). Here's what I've accomplished:
...
- 📋 Claim #CLM-{customer_id}-{datetime.now().strftime('%Y%m%d')}-001234 created
- 📧 Confirmation email sent to {customer_data['email']}
- 🔔 SMS notification sent to registered phone number
...
Is there anything specific about your claim you'd like me to help with, {customer_data['name']}?"""
            ...
            self._simulate_api_call("data-agent", "/mcp/policy/get", "GET", 200, 180, customer_id)
            self._simulate_api_call("data-agent", "/mcp/customer/profile", "GET", 200, 120, customer_id)
            
            customer_policies = customer_data.get('policies', [])
            policy_list = "\n".join([f"   - {policy}" for policy in customer_policies])
            
            demo_response = f"""I've retrieved your comprehensive policy information via A2A orchestration for **{customer_data['name']}** (ID: {customer_id}):

**📄 Customer Overview (Retrieved via MCP):**
- Customer: {customer_data['name']} (ID: {customer_id})
- Email: {customer_data['email']}
- Account Status: {customer_data['status'].title()}
- Active Policies: {len(customer_policies)}

**🛡️ Your Active Policies:**
{policy_list}

**🛡️ Coverage Details (Primary Policy):**
...
- Policy and customer data retrieved securely

Would you like me to explain any specific coverage or help with policy changes, {customer_data['name']}?"""
            ...
            demo_response = f"""Here's your complete billing information via A2A orchestration for **{customer_data['name']}** (ID: {customer_id}):
...
Would you like me to help with any billing changes or have questions about your payments, {customer_data['name']}?"""
            ...
            demo_response = f"""Hello **{customer_data['name']}**! I'm your Insurance AI Assistant with proper A2A/MCP architecture. 

**🔐 Customer Authenticated:**
- Customer ID: {customer_id}
- Name: {customer_data['name']}
- Email: {customer_data['email']}
- Active Policies: {len(customer_data.get('policies', []))}
...
- View your policy details from enterprise systems
...
- View your payment history from financial systems
...
- Coverage explanations with your policy lookup
...
- ✅ Secure customer authentication (You are verified as {customer_data['name']})
...
What would you like help with today, {customer_data['name']}? Just describe your question in natural language!"""
